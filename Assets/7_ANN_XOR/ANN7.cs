...
public class ANN7{
	...
	public double alpha;
	List<Layer7> layers = new List<Layer7>();

	public ANN7(int nI, int nO, int nH, int nPH, double a)
	{
		numInputs = nI;
		numOutputs = nO;
		numHidden = nH;
		...
		if(numHidden > 0)
		{
			layers.Add(new Layer7(numNPerHidden, numInputs));

			for(int i = 0; i < numHidden-1; i++)
			{
				layers.Add(new Layer7(numNPerHidden, numNPerHidden));
			}

			layers.Add(new Layer7(numOutputs, numNPerHidden));
		}
		else
		{
			layers.Add(new Layer7(numOutputs, numInputs));
		}
	}
	...
	double LeakyReLU(double value) //(Leaky Rectified Linear Unit)
	{
		if(value > 0) return value;
		else return 0.01f * value;
	}
	
	double TanH(double value) //(Hyperbolic Tangent)
	{
