...
public class ANN_DeepLearning{
	...
	public double alpha;
	public ActivationFunctionType hiddenActivationFuncType;
	public ActivationFunctionType outputActivationFuncType;
	
	List<Layer_DeepLearning> layers = new List<Layer_DeepLearning>();

	public ANN_DeepLearning(
		int nI,
		int nO,
		int nH,
		int nPH,
		...
		ActivationFunctionType outputActivationFuncType = ActivationFunctionType.Sigmoid)
	{
		...
		this.hiddenActivationFuncType = hiddenActivationFuncType;
		this.outputActivationFuncType = outputActivationFuncType;

		if(numHidden > 0)
		{
			layers.Add(new Layer_DeepLearning(numNPerHidden, numInputs));

			for(int i = 0; i < numHidden-1; i++)
			{
				layers.Add(new Layer_DeepLearning(numNPerHidden, numNPerHidden));
			}

			layers.Add(new Layer_DeepLearning(numOutputs, numNPerHidden));
		}
		else
		{
			layers.Add(new Layer_DeepLearning(numOutputs, numInputs));
		}
	}
	...
	{
		string weightStr = "";
		foreach(Layer_DeepLearning l in layers)
		{
			foreach(Neuron_DeepLearning n in l.neurons)
			{
	...
	{
		return NeuralNetworkTools.GetActivationFunctionByType(value, hiddenActivationFuncType);
	...
	{
		return NeuralNetworkTools.GetActivationFunctionByType(value, outputActivationFuncType);
	}
	
	double GetDerivative(double value)
	{
		return NeuralNetworkTools.GetActivationFunctionDerivativeByType(value, hiddenActivationFuncType);
	}
	
	double GetDerivativeO(double value)
	{
		return NeuralNetworkTools.GetActivationFunctionDerivativeByType(value, outputActivationFuncType);
	}
}
